...
describe("Endfield Power Optimization Calculator", () => {
  ...
  describe("Error Cases (Low Power)", () => {
    it("should return ERROR status if power is below safety margin (195)", () => {
      const result = calculatePowerOptimization(100, BatteryType.LC_WULING);
      expect(result.status).toBe("ERROR");
      expect(result.remainder).toBe(0);
    });

    it("should return ERROR status if power is exactly safety margin (195)", () => {
      const result = calculatePowerOptimization(195, BatteryType.LC_WULING);
      expect(result.status).toBe("ERROR");
    });
  });
  ...
  describe("Optimization Scenarios", () => {
    it("should calculate correct values for LC Wuling(1600) with 2775 power", () => {
      // 2775 - 195 = 2580
      // 2580 % 1600 = 980 (나머지)
      // 상수(32000) / 980 = 32.65...
      // 최적값: 32 (2^5 * 3^0)
      const result = calculatePowerOptimization(2775, BatteryType.LC_WULING);
    ...
    it("should calculate correct values for HC Valley(1100) with 2005 power", () => {
      // 2005 - 195 = 1810
      // 1810 % 1100 = 710 (나머지)
      // 상수(22000) / 710 = 30.98...
      // 최적값: 27 (2^0 * 3^3) -> 32는 30.98보다 크므로 탈락, 27이 선정됨
      const result = calculatePowerOptimization(2005, BatteryType.HC_VALLEY);
  ...
  describe("Direct Connection Cases", () => {
    it("should recommend DIRECT_CONNECTION when remainder is too high", () => {
      ...
      // 입력 전력: 195 + 1400 = 1595
      const result = calculatePowerOptimization(1595, BatteryType.LC_WULING);
  ...
  describe("No Remainder Cases", () => {
    it("should return NO_REMAINDER when power fits exactly", () => {
      // 195 + 1600 = 1795
      const result = calculatePowerOptimization(1795, BatteryType.LC_WULING);
  ...
  // 5. 특수 배터리 (오리지늄: 8초 지속) 테스트
  describe("Special Battery Specs (Originium)", () => {
    it("should handle short duration batteries correctly", () => {
      // 오리지늄: Power 50, Duration 8s
      // 상수 = 50 * (8/2) = 200
      ...
      const result = calculatePowerOptimization(250, BatteryType.ORIGINIUM);
